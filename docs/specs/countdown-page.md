...
- **Header**: logo and the feature name "Travel Countdown" linking to the home page, plus the page actions (**Edit countdown** and **Share**, in that order) when the token is valid.
...
Below the `sm` breakpoint the edit action shows the short label **Editar** and the brand name is hidden, keeping the header on one line down to 320px and with enlarged text. The share action always shows its icon and label. The brand name returns from `md`, where there is room for it next to the actions.
