...
| Header actions (countdown page) | `Button`s in the header, right aligned, 8px gap | **Edit countdown** `text`, then **Share** `contained` with `ShareRounded` icon (primary emphasis); below `sm` both are icon-only (`EditRounded`, `ShareRounded`) with `aria-label`, and the brand name is hidden until `md` |
