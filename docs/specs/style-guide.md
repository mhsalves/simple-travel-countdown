...
| Header actions (countdown page) | `Button`s in the header, right aligned, 8px gap | **Edit countdown** `text`, then **Share** `contained` with `ShareRounded` icon (primary emphasis); the brand name is hidden below `sm` |
