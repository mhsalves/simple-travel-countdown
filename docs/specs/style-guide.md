...
| Header actions (countdown page) | `Button`s in the header, right aligned, 8px gap | **Edit countdown** `text`, then **Share** `contained` with `ShareRounded` icon (primary emphasis); below `sm` the edit action uses the short **Edit** label and the share action is icon-only (`ShareRounded`) with `aria-label`, and the brand name is hidden until `md` |
| Language selector     | Small `TextField select` with `TranslateRounded` icon | Shows `PT` / `EN`; the icon is hidden below `sm` ([Languages](i18n.md)) |
