...
| Header actions (countdown page) | `Button`s in the header, right aligned, 8px gap | **Edit countdown** `text`, then **Share** `contained` with `ShareRounded` icon (primary emphasis); below `sm` the edit action uses the short **Editar** label while the share action keeps its icon and label, and the brand name is hidden until `md` |
